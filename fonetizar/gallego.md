---
name: gallego
type: fonetizar
description: Sistema ortográficamente muy transparente para el hispanohablante, derivado del tronco galaicoportugués.
region: Gallego (Estándar Normativo / ILG-RAG)
---

# gallego

## Perfil Fonético General

El gallego se articula con una fluidez melódica continua, sin la tensión percusiva del español ni la relajación extrema del portugués contemporáneo.

- **Descripción:** Ritmo isosilábico ligado (syllable-timed), pero con una elongación característica de la vocal tónica que genera su famoso "canto".
- **Rasgo Distintivo:** La conservación de las vocales abiertas y cerradas heredadas del latín tardío, y la presencia del fonema postalveolar fricativo /ʃ/ (la grafía 'X').

## Mapeo de Vocales (Reducción al Español)

El gallego cuenta con 7 fonemas vocálicos en posición tónica: /a/, /e/ cerrada, /ɛ/ abierta, /i/, /o/ cerrada, /ɔ/ abierta, /u/. Para el hispanohablante, se transcribirán a las 5 vocales base, pero exigirán mayor apertura mandibular en ciertos casos.

| Patrón / Grafía Original  | Regla de Transformación                                      | Equivalencia Fonética | Ejemplo Práctico     |
| ------------------------- | ------------------------------------------------------------ | --------------------- | -------------------- |
| **a, i, u**               | Idénticas al español                                         | **a, i, u**           | *lúa* → **lú a**     |
| **e** (abierta o cerrada) | Lectura literal (la apertura depende de la raíz etimológica) | **e**                 | *terra* → **té rra** |
| **o** (abierta o cerrada) | Lectura literal                                              | **o**                 | *porta* → **pór ta** |
| **ei, ou**                | Diptongos decrecientes muy frecuentes                        | **éi, óu**            | *cousa* → **kóu sa** |
| **oi, ui**                | Diptongos que exigen no romper la sílaba                     | **ói, úi**            | *moito* → **mói to** |

- **Diptongos clave:** El gallego prefiere mantener los diptongos etimológicos que en castellano monoptongaron (ej. *ou* en lugar de *o*; *ei* en lugar de *e*). Deben leerse fluidos en un solo golpe de voz.

## Mapeo de Consonantes y Sonidos Inexistentes

El inventario consonántico normativo es muy cercano al castellano, pero carece nativamente del sonido de la 'J' velar fuerte (/x/).

| Sonido de Origen (IPA) | Descripción Articulatoria                                                      | Fonetización en Español             |
| ---------------------- | ------------------------------------------------------------------------------ | ----------------------------------- |
| **x** (/ʃ/)            | Fricativa postalveolar sorda. Como la 'sh' inglesa o portuguesa.               | **sh**                              |
| **nh** (/ŋ/)           | Nasal velar. El sonido resuena en el velo del paladar, como la 'ng' en *sing*. | **ng** (suave) o **n** (velarizada) |
| **ll** (/ʎ/)           | Lateral palatal. Idéntica a la 'll' clásica española (no yeísta).              | **ll** (palatal pura)               |
| **ch** (/tʃ/)          | Africada postalveolar sorda.                                                   | **ch**                              |

- **Consonantes mudas:** A diferencia del portugués, el gallego pronuncia todas sus letras ortográficas, careciendo de consonantes mudas puras.

## Reglas de Acentuación, Cadencia y Entonación

La prosodia es el elemento que más delata a un hablante de gallego.

- **Patrones de Acentuación:** Es una lengua predominantemente llana (paroxítona). Las palabras agudas a menudo carecen de la explosión final típica del español.
- **Curva de Entonación ("Canto Galego"):** Al llegar a la sílaba tónica, el tono sube suavemente, la vocal se alarga físicamente de forma sutil, y luego desciende en las sílabas átonas con un arrastre ligado, creando un arco melódico oscilante.
- **Uso de la tilde obligatoria:** Toda palabra fonetizada de más de una sílaba debe llevar explícitamente escrita la tilde en la sílaba tónica.

## Excepciones y "Falsos Amigos" Fonéticos

- **La letra 'X':** Jamás se lee como /ks/ (excepto en cultismos muy específicos normativizados) y jamás como la 'J' del español. La lectura literal hispánica de *Xunta* como *Ksunta* o *Junta* es un error grave; es invariablemente **sh**.
- **El dígrafo 'NH':** Es un falso amigo con el portugués. En portugués representa a la 'ñ' (/ɲ/). En gallego representa a la nasal velar (/ŋ/), utilizada casi exclusivamente en la palabra *unha* (una) y sus derivados.
- **Dialectalismos tolerados (Gheada y Seseo):** Aunque la norma estandariza la 'g' suave y diferencia s/z, amplias zonas de Galicia aplican la *gheada* (pronunciar la 'g' como una 'j' suave o aspirada) y el *seseo* (pronunciar la 'c/z' como 's').

## Casos Prácticos de Aplicación (Lectura Directa)

- **Palabra:** `Xamón` (Jamón)
    - *Desglose:* La 'x' inicial fricativa. Acento agudo.
    - *Fonetización final:* **sha món**

- **Palabra:** `Unha` (Una)
    - *Desglose:* El dígrafo 'nh' indica una nasal velar (la lengua no toca el paladar, sube hacia la campanilla). Suena como una transición fluida nasalizada.
    - *Fonetización final:* **ú nga** (con 'g' muy suave/nasal)

- **Palabra:** `Muller` (Mujer)
    - *Desglose:* Conservación de la 'll' palatal. La 'e' es tónica abierta.
    - *Fonetización final:* **mu llér**

---

## Interferencia Fonética (Simulación de Acento Gallego en Español)

El contacto bilingüe en Galicia produce un sustrato fonético inconfundible al hablar español, caracterizado por el calco entonativo, el cerramiento vocálico átono y la transferencia de fonemas dialectales.

### Distorsión Vocálica (Cerramiento Átono y Elongación)

- **Cerramiento Vocálico Átono:** Tendencia a cerrar sutilmente las vocales 'e' y 'o' cuando se encuentran al final de la palabra y son átonas. La 'o' final se aproxima a una 'u' débil, y la 'e' final a una 'i' débil (ej. *queso* → **ké su**).
- **Elongación Tónica:** La vocal de la sílaba tónica se estira en el tiempo para acomodar la curva melódica ascendente-descendente (ej. *mañana* → **ma ñá- na**).

### Sustitución Consonántica y Simplificación

- **Velarización de la 'N' Final:** Toda 'n' a final de palabra en español se pronuncia velarizada (como si llevara una pequeña 'g' implícita que resuena en la nariz) (ej. *pan* → **páng**).
- **Simplificación de Clústeres (Cultismos):** Fuerte tendencia histórica a simplificar grupos consonánticos cultos del español que el gallego patrimonial redujo (ej. *acto* → **á to**; *perfecto* → **per fé to** o **per féi to**).
- **Transferencia de la Gheada (Dialectal):** Los hablantes de zonas occidentales aplicarán la aspiración faríngea a la 'g' del español (ej. *gato* → **já to** aspirado).

### Alteración Rítmica

- **Cadencia (El Canto):** Imposición de la entonación oscilante gallega sobre las oraciones declarativas españolas, eliminando el tono plano y descendente típico de la meseta castellana. Las frases suelen terminar con un tono ligeramente suspendido o ascendente en lugar de un desplome tajante.

### Casos Prácticos de Aplicación (Lectura Extranjerizada)

- **Palabra Española:** `Gato`
    - *Desglose de Interferencia:* Aplicación dialectal de la *Gheada* sobre la oclusiva velar 'g'. Cerramiento átono de la 'o' final hacia la 'u'. Elongación de la 'a' tónica.
    - *Fonetización simulada:* **já- tu** (con 'j' muy exhalada/suave).

- **Palabra Española:** `Dirección`
    - *Desglose de Interferencia:* Simplificación del grupo consonántico oculto ('cc' se reduce a 'c'). Velarización de la 'n' final.
    - *Fonetización simulada:* **di re sióng** (o **di re zióng** sin seseo).

- **Palabra Española:** `Poco`
    - *Desglose de Interferencia:* Apertura excesiva de la primera 'o' por transferencia del vocalismo de siete grados, y cerramiento de la segunda 'o' átona.
    - *Fonetización simulada:* **pó- ku**
