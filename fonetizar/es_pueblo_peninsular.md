---
name: es_pueblo_peninsular
type: fonetizar
description: No se trata de un idioma ni de un dialecto geográfico estricto, sino de un sociodialecto (o registro de instrucción popular) extendido por las áreas rurales de la mitad norte y centro de la Península Ibérica (Castilla, León, Aragón, etc.). Se caracteriza por la conservación de arcaísmos, la simplificación de grupos consonánticos cultos y la alteración silábica mediante vulgarismos estructurales (metátesis y epéntesis).
region: Habitante de Pueblo (Sociodialecto Rural Genérico Peninsular)
---

# es_pueblo_peninsular

## Perfil Fonético General

El registro rural peninsular exige una articulación recia, con gran tensión en la cavidad bucal posterior (fricativas velares fuertes) y un marcado uso de los labios para resolver hiatos.

- **Descripción:** Ritmo grave y pausado, con aperturas vocálicas amplias y una 's' apicoalveolar sibilante muy densa (silbo cóncavo).
- **Rasgo Distintivo:** La economía articulatoria en sílabas complejas (reducción de consonantes trabadas) contrastando con la inserción de fonemas "parásitos" (epéntesis) para evitar hiatos o facilitar la pronunciación de extranjerismos.

## Mapeo de Vocales (Reducción al Español)

El sistema base es idéntico al estándar, pero sufre alteraciones sistemáticas en las vocales átonas y en los encuentros vocálicos.

| Patrón / Grafía Original       | Regla de Transformación                               | Equivalencia Fonética | Ejemplo Práctico             |
| ------------------------------ | ----------------------------------------------------- | --------------------- | ---------------------------- |
| **e, o** (átonas)              | Cerramiento vocálico (se elevan hacia 'i', 'u')       | **i, u**              | *medicina* → **mi di sí na** |
| **ea, eo, oa** (hiatos)        | Destrucción del hiato creando un diptongo forzado     | **ia, io, ua**        | *peor* → **piór**            |
| **a** (inicial ante 'r' o 'm') | Prótesis (adición de 'a' de apoyo)                    | **a**                 | *radio* → **a rrá dio**      |
| **Vocal + Vocal** (fuertes)    | Inserción de consonante de transición ('g', 'b', 'y') | **V + Cons. + V**     | *toalla* → **to bá ya**      |

- **Diptongos clave:** El habla rural aborrece el hiato (dos vocales fuertes juntas). Automáticamente cierra la primera vocal para convertirla en semiconsonante y fundir ambas sílabas en un solo golpe percusivo (ej. *teatro* → **tiá tro**).

## Mapeo de Consonantes y Sonidos Inexistentes

El inventario es puramente castellano, pero redistribuido mediante fenómenos de reestructuración silábica.

| Sonido de Origen (IPA)  | Descripción Articulatoria                                             | Fonetización en Español |
| ----------------------- | --------------------------------------------------------------------- | ----------------------- |
| **bue / hue / gue**     | Velarización inicial. Fuerte cierre de la garganta.                   | **güe**                 |
| **pt, ct, gn, x**       | Simplificación del grupo culto (asimilación).                         | **t, n, s**             |
| **d** (intervocálica)   | Síncopa (elisión total, especialmente en participios).                | **Ø** (muda)            |
| **s** (final implosiva) | A diferencia del andaluz, la 's' suele conservarse rústica e intacta. | **s** (fuerte)          |
| **j / g** (+e, i)       | Fricativa velar sorda muy áspera y carrasposa.                        | **j** (carraspeada)     |

- **Consonantes mudas / añadidas:** Frecuente **metátesis** (cambio de lugar de las consonantes). La 'r' suele viajar de una sílaba a otra para acomodarse donde exige menos esfuerzo (ej. *Gabriel* → **gra biél**; *Pobreza* → **pro bé sa**).

## Reglas de Acentuación, Cadencia y Entonación

- **Patrones de Acentuación:** Tendencia a la analogía popular. Las palabras esdrújulas a menudo se llanifican por comodidad métrica (*médico* → **me dí ko**), o bien se crean esdrújulas falsas (*pantano* → **pán ta no**). Los verbos con pronombres enclíticos sufren traslación de la 'n' plural (*siéntense* → **sién ten sen**).
- **Curva de Entonación:** Declarativa amplia y robusta. Finales de frase sostenidos o con leves alargamientos vocálicos reflexivos, aportando un tono sentencioso.
- **Uso de la tilde obligatoria:** Toda palabra fonetizada de más de una sílaba debe llevar explícitamente escrita la tilde en la sílaba tónica.

## Excepciones y "Falsos Amigos" Fonéticos

- **La Hipercorrección (Falso Amigo Evolutivo):** El hablante rural, sabedor de que "se come" las letras (como la 'd' en *-ao*), a menudo intenta hablar "fino" introduciendo consonantes donde históricamente nunca las hubo. (ej. *Bacalao* → **ba ka lá do**; *Bilbao* → **bil bá do**).
- **La letra 'X':** Casi nunca se articula como el clúster culto /ks/. Se reduce matemáticamente a una 's' tensa y alargada (ej. *Taxi* → **tá si**; *Examen* → **e sá men**).

## Casos Prácticos de Aplicación (Lectura Directa)

- (Nota: En este contexto, la "lectura directa" asume la pronunciación de vocabulario ya fosilizado en este sociodialecto)*

- **Palabra:** `Abuelo`
    - *Desglose:* Refuerzo velar de la semiconsonante 'u', transformando el inicio en 'g'. Absorción de la 'a' si el flujo es rápido, o mantenimiento de un hiato rústico.
    - *Fonetización final:* **a güé lo**

- **Palabra:** `Cocreta` (Vulgarismo asentado de Croqueta)
    - *Desglose:* Metátesis de la vibrante 'r', que abandona la primera sílaba y se incrusta en la segunda por asimilación al prefijo 'co-'.
    - *Fonetización final:* **ko kré ta**

- **Palabra:** `Dotor` (Doctor)
    - *Desglose:* Simplificación del clúster consonántico culto 'ct'. La 'c' (oclusiva velar) desaparece en favor de la 't' (oclusiva dental).
    - *Fonetización final:* **do tór**

---

## Interferencia Fonética (Simulación de Acento Rural en Español)

Esta sección detalla cómo el hablante rural filtra e interpreta la norma culta del español al leer o hablar, aplicando su matriz de vulgarismos, arcaísmos y economía articulatoria.

### Distorsión Vocálica (Cierre y Diptongación Forzada)

- **Cerramiento Átono Inconsciente:** Sustitución de 'e' por 'i' y de 'o' por 'u' en las sílabas que preceden a la tónica.
- **Destrucción de Hiatos Cultos:** Incapacidad o rechazo articulatorio a mantener dos vocales fuertes contiguas en sílabas separadas.

### Sustitución Consonántica y Fricación

- **Caída del Participio (-ado):** Elisión universal y sistemática de la 'd' intervocálica en todas las palabras terminadas en '-ado', resultando en un diptongo '-ao' con fuerte elongación de la 'a'.
- **Aglutinación y Prótesis:** Inserción de vocales o sílabas de apoyo al inicio de palabra, frecuentemente fusionando el artículo con el sustantivo (ej. *la moto* → **la - a mó to**).

### Alteración Rítmica

- **Cadencia:** Ritmo machacón, de isocronía silábica muy marcada pero entorpecido por micropausas reflexivas. Uso de alargamientos en las conjunciones o preposiciones mientras se piensa la siguiente frase (ej. *y luégo... pué-s...*).

### Casos Prácticos de Aplicación (Lectura Extranjerizada/Dialectal)

- **Palabra Española Estándar:** `Peinado`
    - *Desglose de Interferencia:* Aplicación de la síncopa en el sufijo participial. Caída de la 'd' intervocálica.
    - *Fonetización simulada:* **pei ná o**

- **Palabra Española Estándar:** `Electricidad`
    - *Desglose de Interferencia:* Simplificación del grupo culto 'ct'. Metátesis o cerramiento de la 'e' inicial. Elisión de la 'd' final.
    - *Fonetización simulada:* **e le tri si dá-** (o **i le tri si dá-**)

- **Palabra Española Estándar:** `Indigestión`
    - *Desglose de Interferencia:* Sustitución de la fricativa velar culta por un vulgarismo analógico. Alteración prefijal.
    - *Fonetización simulada:* **in di jes tión** (o frecuentemente transformado en **in di ges tión** con 'g' dura por hipercorrección).
