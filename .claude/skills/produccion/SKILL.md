...
- **Entrada por Mapas:** Cada skill delegada entra por el mapa `.claude/rules/*.md` que le corresponda (no siempre uno con su mismo nombre — `fusionar` entra por el de `style_box`) y abre sus manuales de `composicion/` bajo demanda. Las skills orquestadoras o de proceso (`proyecto`, `retrospectiva`, `cover_art`) no tienen mapa ni manual propio. `produccion` **no** lee manuales `composicion/` directamente, salvo `exclude_box` y `formato`, que no tienen skill propia y se resuelven en Fase 5.
