...
2. **Fase 1 — style_box (borrador).**
    ...
    - Construye el borrador del `style_box`, combinando canon + instinto + `composicion/style_box.md` (máx ~20 palabras). (Apóyate en las skills `style-box`, `buscar-tag` y `fusionar`).
    - **PARA.**
3. **Fase 2 — Letra (borrador).**
    - Redacta la letra sin tags, foco poético/narrativo; queda abierta a refinarse en fases posteriores. Usa `composicion/letra.md` y la skill `lirica`.
...
4. **Fase 3 — lyrics_box (borrador).**
    - Inyecta las tags de estructura y dirección de banda de manera magistral en el `lyrics_box`, busca por concepto (grep, solo coincidencias) en `composicion/lyrics_box.md` y `composicion/tecnicas_vocales.md`.
    ...
    - Reajusta o modifica la letra si un tag o un fraseo mejora el resultado. Apóyate en la skill `buscar-tag`.
    - Crea `[MOOD]` y `[PRODUCTION]`.
    - **PARA.**
5. **Fase 4 — Masterización.**
    - Abre `<razonamiento>`, eleva el impacto y controla el post-procesamiento de `efectos` con `composicion/efectos.md`; construye el `exclude_box` con `composicion/exclude_box.md`.
