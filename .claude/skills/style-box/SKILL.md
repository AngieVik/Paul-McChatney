...
2. Busca por concepto (grep, solo coincidencias) dentro del núcleo de `chupilista/`.
  (sobre todo 01 género, 03 instrumentación, 04/12 voz, 05 ritmo, 08 dinámica, 10 experimental). La skill `buscar-tag` automatiza esta búsqueda.
...
5. Genera una línea de `exclude_box` separados por coma,) bloqueando clichés e instrumentos no deseados.
