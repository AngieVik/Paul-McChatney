...
**Ronda 3 (chequeo 3 reescrito — tags línea a línea):** el chequeo anterior sumaba `[` y `]` de todo el bloque de código; una tag mal cerrada podía quedar invisible si otra tag distinta compensaba el conteo total. Se sustituyó por un análisis por línea con pila de profundidad (`check_bracket_tags`): cada línea de cada bloque se recorre carácter a carácter y reporta, con número de línea y fragmento exacto, tres casos — `]` sin `[` previo en esa misma línea, `[` que no cierra antes de terminar la línea, y corchetes anidados (profundidad > 1, no usados en la sintaxis de tags de este proyecto). Se recalibró contra todo el árbol real: cero avisos nuevos de este tipo (los tags reales del proyecto están bien formados línea a línea), confirmando que el cambio no introduce ruido. La misma ejecución sacó a la luz un hallazgo real no relacionado con el punto 3: además de `el_gato_de_carabas.md`, otros siete proyectos (`el_camino_del_justo`, `el_cartero_que_no_llega`, `el_coplista_de_la_luna_llena`, `el_jambo_tieso`, `el_legionario_y_la_morita_cover`, `el_morito_juan_cover`, `god_of_the_cave`) tienen el mismo patrón de \`\`\`yaml sin cerrar en la línea 27 — probablemente heredado de una versión antigua de `plantilla_proyecto.md`. Son obras cerradas: se documenta aquí, no se tocan sin petición explícita del usuario.
