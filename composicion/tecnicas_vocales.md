...
- *Desambiguación (regla):* si el paréntesis contiene una **palabra u onomatopeya** (`(oleee)`, `(¡ay!)`, `(all)`), es voz secundaria **cantada** y suma sílabas. Si contiene un **término técnico de entrega** de la lista cerrada de §2.4 (`(whispered)`, `(shouted)`, `(conversational)`, `(borderline shouted)`, `(breathy)`), es **dirección de performance**: no se canta ni suma sílabas.*
    - **Coros y apoyos:** 
        * **Ejemplo:**
...
### 2.4 · Dirección de Interpretación Dinámica
- *A diferencia de las etiquetas de estilo globales, el motor responde con alta precisión a comandos de actuación inyectados directamente entre las líneas de la letra.*
