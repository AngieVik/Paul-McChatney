---
name: chupilista
type: map
description: Índice del canon local de tags y modificadores, organizado en 15 módulos especializados.
---

- *Este índice es el mapa concepto→archivo; abre bajo demanda solo el/los módulos de `chupilista/` que necesites.*
- **Canon local no equivale a eficacia demostrada:** confirma grafía y procedencia; solo una prueba registrada permite llamarla observada en Suno.
- **Consulta por búsqueda (grep) o salto por Índice.**

---


# chupilista

## Indice

- Indice 01_core_genres_and_subgenres
`1 · Música Electrónica, Dance y Derivados Sintéticos`
`2 · Rock, Metal, Punk y Vertientes Alternativas`
`3 · Hip-Hop, Rap, Trap y R&B Contemporáneo`
`4 · Pop, Indie y Synthpop`
`5 · Jazz, Blues, Funk y Soul`
`6 · Folk, Country, Americana y Raíces Tradicionales`
`7 · Música Latina, Caribeña y Reggae`
`8 · Música Africana y Afro-Fusión`
`9 · Música Asiática, Medio Oriente y Global`
`10 · Música Clásica, Coral y Tradición Europea`
`11 · Ambient, Experimental, Vanguardia y Paisajes Sonoros`
`12 · Cibernética, Chiptune y Cultura de Videojuegos`

- Indice 02_atmosphere_and_mood
`1 · Ingeniería Sonora y Texturas`
`2 · Sci-Fi, Ciberespacio y Vacío Cósmico`
`3 · Tensión, Paranoia y Terror Psicológico`
`4 · Melancolía, Luto y Desolación`
`5 · Naturaleza Extrema y Elementos`
`6 · Ruina Industrial y Decadencia Urbana (Industrial, Urban Decay & Ruins)`
`7 · Intimidad, Romance y Euforia`
`8 · Furia, Acción y Caos Adrenalínico`
`9 · Misticismo, Sueños y Surrealismo`

- Indice 03_instrumentation_and_stems
`1 · Percusión y Baterías`
`2 · Frecuencias Graves`
`3 · Guitarras y Cuerdas Pulsadas`
`4 · Instrumentos de Teclado`
`5 · Sintetizadores y Electrónica`
`6 · Sección Orquestal y Cuerdas Frotadas`
`7 · Vientos y Metales`
`8 · Voces y Texturas Corales`
`9 · Diseño Sonoro, Texturas y Foley`
`10 · Marcadores Estructurales`

- Indice 04_vocal_persona_and_timbre
`1 · Clasificación Vocal Clásica y Tonalidad`
`2 · Identidad, Edad y Género`
`3 · Emociones, Estado y Condición Física`
`4 · Géneros Musicales y Estilos`
`5 · Técnicas Vocales, Textura y Expresión`
`6 · Coros, Agrupaciones y Armonías`
`7 · Personajes, Arquetipos y Locuciones`
`8 · Efectos, Procesamiento y Entorno Acústico`

- Indice 05_rhythm_and_tempo
`1 · Tempo, Velocidad y Multiplicadores`
`2 · Compases, Polirritmias y Rejillas Matemáticas`
`3 · Groove, Cuantización y Micro-Timing`
`4 · Patrones de Género y Estilos Específicos`
`5 · Glitch, Stutter y Fracturas IDM`
`6 · Dinámica, Acústica Espacial y Estructura Rítmica`

- Indice 06_song_structure_and_sections
`1 · Introducciones y Preámbulos`
`2 · Versos y Estrofas`
`3 · Pre-Estribillos y Construcciones de Tensión`
`4 · Estribillos y Ganchos (Hooks)`
`5 · Puentes y Breakdowns`
`6 · Interludios, Drops y Transiciones`
`7 · Outros, Codas y Finales`
`8 · Narrativa, Motivos y Elementos Conceptuales`

- Indice 07_music_theory_harmony_and_scales
`1 · Afinación, Frecuencias y Sistemas Microtonales`
`2 · Acordes, Voicings y Estructuras Armónicas`
`3 · Progresiones, Cadencias y Movimientos`
`4 · Escalas occidentales, Modos y Conceptos Teóricos`
`5 · Escalas Exóticas y Sistemas del Mundo`
`6 · Taxonomía Aplicada por Tonalidad Fundamental`

- Indice 08_dynamics_and_intensity
`1 · Impactos, Golpes y Explosiones`
`2 · Caídas, Silencios y Cortes`
`3 · Subidas, Crescendos y Transiciones`
`4 · Clímax, Máxima Intensidad y Muros de Sonido`
`5 · Tensión, Dinámicas Bajas y Sutileza`
`6 · Acentuaciones y Dinámicas Rítmicas`
`7 · Compresión, Limitación y Procesamiento Dinámico`
`8 · Dinámicas Vocales, Ambientales e Instrumentales Específicas`

- Indice 09_foley_and_sound_design_fx
`1 · Ambiente y Naturaleza`
`2 · Animales y Criaturas`
`3 · Humanos y Voces`
`4 · Impactos, Armas y Destrucción`
`5 · Foley Doméstico y Cotidiano`
`6 · Tecnología, Máquinas y Vehículos`
`7 · Efectos, Transiciones y Texturas Acústicas`

- Indice 10_experimental_modes
`1 · Procesamiento Algorítmico, Generativo y Estocástico`
`2 · Manipulación Temporal, Métrica y Rítmica`
`3 · Alteración de Fase, Psicoacústica y Espacialización`
`4 · Microtonalidad, Disonancia y Deriva Estructural`
`5 · Corrupción Digital, Glitch y Degradación de Datos`
`6 · Síntesis, Filtrado y Procesamiento Espectral Granular`
`7 · Saturación, Ruido de Fondo y Entornos Industriales`
`8 · Manipulación de Arreglos, Narrativa y Formantes Vocales`

- Indice 11_production_and_effect
`1 · Dinámica y Volumen`
`2 · Ecualización y Filtros`
`3 · Saturación, Distorsión y Lo-Fi`
`4 · Reverberación y Espacios`
`5 · Delay y Ecos`
`6 · Modulación`
`7 · Tono, Voces y Formantes`
`8 · Imagen Estéreo y Espacialidad`
`9 · Glitch, Stutter y Efectos Experimentales`
`10 · Mezcla Base y Utilidades Generales`

- Indice 12_vocal_delivery_and_expressivity
`1 · Estilos Agresivos, Metal y Distorsión Extrema`
`2 · Técnicas Íntimas, Susurros y ASMR`
`3 · Canto Clásico, Coral y Operístico`
`4 · Emociones Extremas, Desesperación y Llanto`
`5 · Procesamiento Digital, Glitch y Voces Robóticas`
`6 · Rap, Spoken Word y Rítmica Acelerada`
`7 · Canto Étnico, Tradicional y Canto Gutural`
`8 · Voz Hablada, Monólogos y Actuación Teatral`
`9 · Efectos de Estudio, Capas y Modulación de Tono`
`10 · Soul, R&B, Blues y Melodías Expresivas`

- Indice 13_advanced_modifiers_allowed
`1 · Procesamiento Vocal y Formantes`
`2 · Dinámica, Compresión y Sidechain`
`3 · Ecualización, Filtros y Frecuencias`
`4 · Espacialidad y Reverberación`
`5 · Modulación, Retardo y Campo Estéreo`
`6 · Saturación, Distorsión y Degradación Lo-Fi`
`7 · Tiempo, Ritmo y Estructura`
`8 · Síntesis, Instrumentación y Tonalidad`
`9 · Matrices Cuánticas y Experimentales`

- Indice 14_nudging_and_callbacks
`1 · Callbacks y Retornos Estructurales`
`2 · Gestión de Pistas y Arreglos`
`3 · Dinámica, Tempo y Modulación Rítmica`
`4 · Efectos de Audio, Espacio y Texturas`
`5 · Tratamiento Vocal y Coros`
`6 · Instrumentación, Género y Motivos`

- Indice 15_negative_prompts_and_exclude_styles
`1 · Géneros, Estilos y Estructuras Musicales`
`2 · Instrumentos, Arreglos y Composición`
`3 · Voces y Tratamiento Vocal`
`4 · Efectos Espaciales (Reverb y Delay)`
`5 · Efectos de Modulación y Afinación`
`6 · Distorsión, Saturación y Estética Lo-Fi`
`7 · Dinámica, Ecualización y Mezcla Espacial`
`8 · Artefactos, Ruidos y Edición`

## Módulos

| Archivo                                                                                                              | Contenido                                                                        |
| -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| <a href="../../chupilista/01_core_genres_and_subgenres.md">`01_core_genres_and_subgenres.md`</a>                     | Géneros y subgéneros: el vocabulario base para definir el estilo principal.      |
| <a href="../../chupilista/02_atmosphere_and_mood.md">`02_atmosphere_and_mood.md`</a>                                 | Atmósfera y mood: tags de ambiente, emoción y textura emocional.                 |
| <a href="../../chupilista/03_instrumentation_and_stems.md">`03_instrumentation_and_stems.md`</a>                     | Instrumentación y stems: instrumentos específicos, layers y arreglos.            |
| <a href="../../chupilista/04_vocal_persona_and_timbre.md">`04_vocal_persona_and_timbre.md`</a>                       | Persona vocal y timbre: tipo de voz, carácter, edad, textura y presencia.        |
| <a href="../../chupilista/05_rhythm_and_tempo.md">`05_rhythm_and_tempo.md`</a>                                       | Ritmo y tempo: BPM, subdivisiones, grooves, feels rítmicos.                      |
| <a href="../../chupilista/06_song_structure_and_sections.md">`06_song_structure_and_sections.md`</a>                 | Estructura y secciones: tags de sección ([Verse], [Chorus], [Bridge], etc.).     |
| <a href="../../chupilista/07_music_theory_harmony_and_scales.md">`07_music_theory_harmony_and_scales.md`</a>         | Armonía y escalas: modos, tensiones, progresiones y vocabulario tonal.           |
| <a href="../../chupilista/08_dynamics_and_intensity.md">`08_dynamics_and_intensity.md`</a>                           | Dinámica e intensidad: crescendos, drops, silencio, saturación y arco dramático. |
| <a href="../../chupilista/09_foley_and_sound_design_fx.md">`09_foley_and_sound_design_fx.md`</a>                     | Foley y diseño de sonido: efectos ambientales, texturas y samples de entorno.    |
| <a href="../../chupilista/10_experimental_modes.md">`10_experimental_modes.md`</a>                                   | Modos experimentales: fusiones atípicas, técnicas extendidas, glitch y noise.    |
| <a href="../../chupilista/11_production_and_effect.md">`11_production_and_effect.md`</a>                             | Producción y efectos: calidad de mezcla, reverb, delay, compresión y mastering.  |
| <a href="../../chupilista/12_vocal_delivery_and_expressivity.md">`12_vocal_delivery_and_expressivity.md`</a>         | Delivery vocal y expresividad: fraseo, articulación, dinámicas y ornamentos.     |
| <a href="../../chupilista/13_advanced_modifiers_allowed.md">`13_advanced_modifiers_allowed.md`</a>                   | Modificadores registrados en el canon local; su eficacia no se presupone.       |
| <a href="../../chupilista/14_nudging_and_callbacks.md">`14_nudging_and_callbacks.md`</a>                             | Nudging y callbacks: cómo encontrar un resultado específico entre intentos.      |
| <a href="../../chupilista/15_negative_prompts_and_exclude_styles.md">`15_negative_prompts_and_exclude_styles.md`</a> | Prompts negativos y exclusiones: qué excluir del style_box y cómo formularlo.    |

---
